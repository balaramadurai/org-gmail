...
- Reply to and delegate (forward) emails directly from Emacs, with full control over recipients. Sent replies are automatically inserted into the Org thread.
- Create actionable sub-tasks for emails that will appear in your agenda.
- Download and save email attachments to a configurable directory, with Org links for local access and options to view in Gmail.
- Smart syncing: New messages in existing threads are appended to the thread's current Org file location, not the default inbox.
- Automatic reply insertion: Sent replies appear immediately in the Org thread.
 ...
 ** Python Configuration
 ...
 Create a ~config.json~ file in the same directory as ~gmail_label_manager.py~ with settings like:
 ...
 The ~{label}~ placeholder in ~attachment_dir~ will be replaced with the label name (e.g., ~attachments/Work/~ for Work label).
